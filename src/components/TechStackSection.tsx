    ...
    <section className="relative py-20 px-6 overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto max-w-6xl">
        <div className="text-center mb-10 md:mb-16">
          <h2 className="text-4xl sm:text-5xl md:text-6xl font-bold text-glow-blue mb-4">
            Tech Stack
          </h2>
        ...
        <div className="grid grid-cols-2 gap-4 sm:gap-6 max-w-2xl mx-auto">
          {categories.map((category, index) => (
            <div
              ...
              className="bg-card/30 backdrop-blur-sm border border-border rounded-xl p-4 sm:p-8 
                       hover:border-primary/50 transition-all duration-300 cursor-pointer
                       hover:-translate-y-2 group"
            >
              <category.icon className="w-8 h-8 sm:w-12 sm:h-12 text-primary mb-2 sm:mb-4 group-hover:scale-110 transition-transform" />
              <h4 className="font-bold text-base sm:text-xl mb-1 sm:mb-2">{category.label}</h4>
      ...
      <Dialog open={openDialog === "programming"} onOpenChange={() => setOpenDialog(null)}>
        <DialogContent className="bg-card/95 backdrop-blur-xl border-border max-w-[90vw] sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-xl sm:text-2xl font-bold text-center mb-4">Programming Tools</DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-3 gap-3 sm:gap-6">
            {programmingTools.map((tool, index) => (
              <div key={index} className="flex flex-col items-center gap-2 sm:gap-3 p-2 sm:p-4 rounded-xl bg-background/50 hover:bg-background/80 transition-colors">
                <img src={tool.logo} alt={tool.name} className="w-10 h-10 sm:w-16 sm:h-16 object-contain" />
                <span className="font-semibold text-xs sm:text-sm text-center">{tool.name}</span>
              </div>
            ))}
          </div>
        </DialogContent>
      ...
      <Dialog open={openDialog === "design"} onOpenChange={() => setOpenDialog(null)}>
        <DialogContent className="bg-card/95 backdrop-blur-xl border-border max-w-[90vw] sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-xl sm:text-2xl font-bold text-center mb-4">Design Tools</DialogTitle>
          </DialogHeader>
